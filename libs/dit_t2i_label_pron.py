...
class CrossAttention(nn.Module):
    ...
    def forward(self, query, context):
        B, N, C = query.shape        
        q = einops.rearrange(self.ql(query), 'B N (H D) -> B H N D', H=self.num_heads).float()
        k = einops.rearrange(self.kl(context), 'B L (H D) -> B H L D', H=self.num_heads).float()
        v = einops.rearrange(self.vl(context), 'B L (H D) -> B H L D', H=self.num_heads).float()
...
class DiTBlock(nn.Module):
    ...
    """
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, use_checkpoint=False, **block_kwargs):
        super().__init__()
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.context_embed = nn.Linear(clip_dim, hidden_size)
        ...
        if num_classes > 0:
            ...
            self.prototype = nn.Embedding(num_classes, num_patches * hidden_size)
            self.q = nn.Linear(hidden_size, hidden_size)
            self.k = nn.Linear(hidden_size, hidden_size)
            self.v = nn.Linear(hidden_size, hidden_size)
            self.cross_attn = CrossAttention(hidden_size, num_heads=num_heads)
            self.z1 = zero_module(nn.Linear(hidden_size, hidden_size, bias=True))
            self.z2 = zero_module(nn.Linear(hidden_size, hidden_size, bias=True))
        else:
    ...
    def forward(self, x, timesteps, context=None, y=None, x_clean=None):
        """
        Forward pass of DiT.
        ...
        t = self.t_embedder(timestep_embedding(timesteps, self.x_embedder.proj.out_channels))  # (N, D)
        ...
        if y is not None and hasattr(self, 'prototype') and x_clean is not None:
            # Get prototype
            prototype = self.prototype(y).view(x.shape[0], -1, self.x_embedder.proj.out_channels)  # B, N, D
            ...
            recon_latent = torch.nn.functional.scaled_dot_product_attention(
                einops.rearrange(latent_x, 'B L (H D) -> B H L D', H=self.num_heads).float(),
            ...
            # Apply cross attention with zero initialization
            x_attn = self.cross_attn(x, prototype)
            x = x + self.z1(x_attn)
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale, context=None):
        ...
        model_out = self.forward(combined, t, context, y)
...
# Import numpy for positional embedding
import numpy as np
