...
def evaluate(config):
    ...
    def sample_fn(_n_samples):
        ...
        if config.train.mode == 'uncond':
            ...
            labels = dataset.sample_label(_n_samples, device=device)
            kwargs = dict(y=labels)
        else:
    ...
    with tempfile.TemporaryDirectory() as temp_path:
        ...
        utils.sample2dir_labels(accelerator, path, config.sample.n_samples, config.sample.mini_batch_size, sample_fn, dataset.unpreprocess)
