...
def stp(s, ts: torch.Tensor):  # scalar tensor product 实现一个参数和一个维度的所有参数相乘
...
def train(config):
    ...
    if accelerator.is_main_process:
        os.makedirs(config.ckpt_root, exist_ok=True)  #标记一下不知道在哪里定义的
    ...
    # print("num_sampler:", list(sampler))
    ...
    def eval_step(n_samples, sample_steps):
        ...
        def sample_fn(_n_samples):
            if config.train.mode == 'uncond':
                ...
                kwargs = dict(y=dataset.sample_label(_n_samples, device=device)) #获得随机的标签
            else:
                raise NotImplementedError
            return dpm_solver_sample(_n_samples, sample_steps, **kwargs) # 返回采样的图片
    ...
    while train_state.step < config.train.n_steps:
        ...
        if accelerator.is_main_process and train_state.step % config.train.eval_interval == 0:
            ...
            if config.train.mode == 'uncond':
                samples = dpm_solver_sample(_n_samples=15 * 10, _sample_steps=50)
            elif config.train.mode == 'cond':
                y = einops.repeat(torch.arange(15, device=device) % dataset.K, 'nrow -> (nrow ncol)', ncol=10)
                samples = dpm_solver_sample(_n_samples=15 * 10, _sample_steps=50, y=y)
            else:
                raise NotImplementedError
...
config_flags.DEFINE_config_file(
    "config", None, "Training configuration.", lock_config=False)  # 以 absl.flags 提供的机制来注册了一个新参数（--config）
