...
class ScoreModel(object):
    ...
    def predict(self, xt, t, **kwargs):
        ...
        return self.nnet(xt, t * 999, **kwargs)  # follow SDE 为什么要 * 999？猜测可能是continuous time->discritize

    def noise_pred(self, xt, t, **kwargs):
        if kwargs.get('x_clean') is not None:
            pred, recon_loss = self.predict(xt, t, **kwargs)
        else:
        ...
        if self.pred == 'noise_pred':
            ...
            raise NotImplementedError
        return (noise_pred, recon_loss) if kwargs.get("x_clean") is not None else noise_pred
    ...
    def x0_pred(self, xt, t, **kwargs):
        ...
        if self.pred == 'noise_pred':
            ...
            raise NotImplementedError
        return (x0_pred, recon_loss) if kwargs.get('x_clean') is not None else x0_pred
...
def LSimple(score_model: ScoreModel, x0, pred='noise_pred', **kwargs):
    ...
    if pred == 'noise_pred':
        if kwargs.get('x_clean') is not None:
            noise_pred, recon_loss = score_model.noise_pred(xt, t, **kwargs)
            return mos(noise - noise_pred) + recon_loss  ### recon_loss待补充
        else:
