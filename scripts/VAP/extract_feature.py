...
'''
1. transfer the imgs from pixel space to this latent space, 
2. transfer the report from word to the latent space, 
3. save the label at the same time.
'''
...
def main(resolution=256):
    ...
    if args.split == "train":
        ...
        datas = ChestXray14Database(root='/storage/dataset/ChestXray14',
                             ...
                             size=resolution,
                             mode='train')
        save_dir = f'/storage/U-ViT/assets/datasets/VAP/ChestXray14-{resolution}_features-{version.split("/")[-1]}/train'
    elif args.split == "val":
        # datas = ChestXray14Database(root='/cpfs01/projects-HDD/cfff-906dc71fafda_HDD/gbw_21307130160/datasets/ChestXray14',
        ...
        datas = ChestXray14Database(root='/storage/dataset/ChestXray14',
                             ...
                             size=resolution,
                             mode='valid')
        save_dir = f'/storage/U-ViT/assets/datasets/VAP/ChestXray14-{resolution}_features-{version.split("/")[-1]}/val'
    else:
    ...
    with torch.no_grad():
        for idx, data in tqdm(enumerate(datas), total=len(datas)):
            ...
            for i in range(len(latent)):
                ...
                np.save(os.path.join(save_dir, f'{idx}_{i}.npy'), {"caption":c, "label":l})
